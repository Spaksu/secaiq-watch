...
function banners(d) {
  ...
  if (!DEMO && P.os === 'windows') {
    const au = d.audit_status === 'ok' ? '<b>on</b>' : d.audit_status === 'denied' ? '<b>not readable</b> (your user must be in <i>Event Log Readers</i>; sign out and in after adding it)' : d.audit_status === 'empty' ? '<b>no records yet</b> (auditing may not be switched on)' : '<b>off</b>';
    b.push(['sky', `Windows mode: Windows exposes <b>no per-connection byte counters</b>, so charts show <b>open connections</b> and byte figures show “–”. It also has no full open-file list: the Files tab shows which AI tool holds a <b>known credential file</b> open (SSH keys, cloud CLI credentials, browser password and cookie databases …) and, if an administrator enabled file auditing, every read of those folders. File auditing: ${au}. Set it up once with <code class="mono">bin\\windows-file-audit.ps1 enable</code> in an elevated PowerShell (see the User guide).`]);
  }
